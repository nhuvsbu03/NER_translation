...
print("Loading model (plain BF16, no quantization, device_map=cuda, full 80GB VRAM)...")
t0 = time.time()
...
def translate(text, src_lang, tgt_lang, max_new_tokens=150):
    ...
    full = tokenizer.decode(seqs[0][input_len:], skip_special_tokens=True)
    return full.strip()


srcs = [r["source"] for r in rows]

print(f"\nRunning {len(srcs)}-sentence timed test (plain BF16, full VRAM, no offload)...")
times = []
...
print(f"=== Overall avg (all {len(times)} calls): {overall:.2f}s ===")
