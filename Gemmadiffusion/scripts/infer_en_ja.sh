#!/bin/bash
# Usage: bash scripts/infer_en_ja.sh [MAX_LINES]
...
python3 "$REPO_DIR/analysis/infer_diffgemma.py" \
    --src_file "$REPO_DIR/data/en-ja/test.en" \
    --ref_file "$REPO_DIR/data/en-ja/test.ja" \
    --out_dir  "$REPO_DIR/results/en-ja" \
    --src_lang "English" --tgt_lang "Japanese" $MAX_ARG
