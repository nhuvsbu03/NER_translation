#!/bin/bash
# Usage: bash scripts/infer_ja_en.sh [MAX_LINES]
...
python3 "$REPO_DIR/analysis/infer_diffgemma.py" \
    --src_file "$REPO_DIR/data/ja-en/test.ja" \
    --ref_file "$REPO_DIR/data/ja-en/test.en" \
    --out_dir  "$REPO_DIR/results/ja-en" \
    --src_lang "Japanese" --tgt_lang "English" $MAX_ARG
