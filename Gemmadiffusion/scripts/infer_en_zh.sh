#!/bin/bash
# Usage: bash scripts/infer_en_zh.sh [MAX_LINES]
...
python3 "$REPO_DIR/analysis/infer_diffgemma.py" \
    --src_file "$REPO_DIR/data/en-zh/test.en" \
    --ref_file "$REPO_DIR/data/en-zh/test.zh" \
    --out_dir  "$REPO_DIR/results/en-zh" \
    --src_lang "English" --tgt_lang "Chinese" $MAX_ARG
