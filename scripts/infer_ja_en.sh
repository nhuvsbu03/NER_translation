#!/usr/bin/env bash
# Run SeqDiffuSeq inference on the JA-EN test set.
# Supports targeting a specific checkpoint step for staged evaluation.
# Usage (from /root/NER_translation):
#   bash scripts/infer_ja_en.sh           → uses latest EMA checkpoint
#   bash scripts/infer_ja_en.sh 5000      → targets ema_0.9999_005000.pt
#   bash scripts/infer_ja_en.sh 25000     → targets ema_0.9999_025000.pt
set -euo pipefail
...
if [ -n "$STEP" ]; then
    ...
    if [ -z "$EMA_CKPT" ]; then
        ...
        echo "       Training must complete at least one save_interval (5,000 steps)."
        exit 1
    fi
...
TEST_FILE="$REPO_DIR/data/ja-en/test.ja"
NUM_TEST=$(wc -l < "$TEST_FILE")
echo "==> Test set:   $NUM_TEST sentences"
echo ""
...
echo "    Pull results with: .\\scripts\\pull_results.ps1 -Pair ja-en (from Windows)"
